...
describe('Read value of id 1 from fixture using forEach', () => {
    ...
    it('Fetches the user with id 3', function () {
        ...
        cy.fixture('users').then((dataValue)=>{
            ...
            let userWithId1 = null;
  
      // Use forEach to iterate through the array
       dataValue.forEach((Duser) => {
        if (Duser.id === 1) {
            userWithId1 = Duser;
        }
      });
      ...
      // Assertions or usage
      expect(userWithId1).to.not.be.null;
    //   expect(userWithId3.name).to.equal('Michael Brown');
    //   expect(userWithId3.email).to.equal('michael.brown@example.com');
  
      // Log the user details
      cy.log('User with ID 1:', userWithId1);
      cy.log(userWithId1.address1.location)
